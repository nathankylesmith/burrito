...
const fetchMenuText = async (source, logger) => {
...
const parseDishResponse = (content) => {
...
const callMenuModel = async (menuText, restaurantName, options) => {
    ...
    const payload = {
        ...
        messages: [
            {
                role: 'system',
                content: 'Extract a concise list of dishes from the provided restaurant menu text. Return JSON array with objects: {"name": string, "description": string | null, "price": number | null, "menu_section": string | null}. Keep descriptions under 200 characters.',
            },
            {
                role: 'user',
...
const fallbackDishesFromText = (text) => {
    const candidates = [];
...
const buildDishTemplates = (dishes, placeId, source) => dishes.map((dish, index) => ({
    placeId,
...
const buildRestaurantPayload = (source, regionId, placeId) => ({
    region_id: regionId,
    place_id: placeId,
    name: source.name,
    description: null,
...
const hashPlaceId = (source) => source.placeId || `menu-${createHash('sha1').update(`${source.menuUrl}:${source.name}`).digest('hex').slice(0, 16)}`;
export const loadRestaurantsFromMenuSources = async (options) => {
    ...
    for (const source of options.restaurants) {
        ...
        try {
            ...
            placeLogger.warn('Skipping menu source due to error', { error: error.message });
        }
    }
    let loadResult;
    try {
