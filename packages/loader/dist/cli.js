...
program
    ...
    .option('--min-dish-photo-confidence <value>', 'Minimum confidence (0-1) required to accept a photo as a dish', asNumber('min-dish-photo-confidence'), 0.35)
    ...
    const loaderMode = options.mode || 'google';
    const isGoogleMode = loaderMode === 'google';
    const isHybridMode = loaderMode === 'hybrid';
    const llmProvider = (options.llmProvider || 'local').toLowerCase();
    ...
    let resolvedLlmEndpoint = options.llmEndpoint ||
    ...
    try {
        ...
        if (lastRefreshedAt &&
            ...
            console.log(`Skipping refresh for region ${existingRegion?.id || existingRegion?.region_key} because it was refreshed at ${lastRefreshedAt.toISOString()}`);
            process.exit(0);
        }
        if (sinceDate && lastRefreshedAt && lastRefreshedAt >= sinceDate) {
            console.log(`Skipping refresh: last refresh (${lastRefreshedAt.toISOString()}) is newer than --since (${sinceDate.toISOString()}).`);
            process.exit(0);
        }
        ...
        const loaderLogger = createLogger('cli-runner', { cli: true }, { level: options.verbose ? 'debug' : undefined });
