...
import { defaultLogger, withContext } from './logger.js';
...
const upsertRawPhotoRecord = async (supabase, logger, record) => {
    try {
        const payload = {
...
const extractExternalPhotoInputs = (details) => {
    const candidates = [];
...
const downloadExternalPhoto = async (url, logger, meta) => {
    try {
...
const buildPhotoDishName = (insight, cuisines, index) => {
    const base = insight?.dish_guess?.trim() ||
        insight?.alternate_names?.find((name) => name && name.trim()) ||
        `${cuisines[0] || 'Signature'} Dish ${index + 1}`;
    return base.trim();
};
const buildPhotoDishDescription = (insight, restaurantName) => {
    const parts = [
        insight?.caption,
...
const pickDishPrice = (insight, priceLevel, index) => {
...
const ingestExternalPhotoDishes = async ({ details, context, cuisines, gallery, placeLogger, }) => {
    ...
    for (const candidate of candidates) {
        ...
        const allowedSourceTypes = ['review', 'photo', 'menu', 'fallback'];
        const sourceType = allowedSourceTypes.includes(candidate.sourceType)
        ...
        const uploadedUrl = await uploadDishPhoto(context.supabase, photoData, details.place_id, photoReference, context.bucket, placeLogger);
        if (!uploadedUrl) {
            await upsertRawPhotoRecord(context.supabase, placeLogger, {
                placeId: details.place_id,
                photoReference,
...
const generatePhotoDishTemplates = async ({ details, context, cuisines, gallery, placeLogger, }) => {
    if (!context.visionClient || !context.visionClient.classifyDishPhoto) {
    ...
    const desiredCount = Math.max(1, context.maxDishPhotosPerRestaurant ?? context.maxDishesPerRestaurant ?? 5);
    ...
    for (let i = 0; i < scanLimit && collected.length < desiredCount; i += 1) {
        ...
        const uploadedUrl = await uploadDishPhoto(context.supabase, photoData, details.place_id, photoRef, context.bucket, placeLogger);
        if (!uploadedUrl) {
            await upsertRawPhotoRecord(context.supabase, placeLogger, {
                placeId: details.place_id,
                photoReference: photoRef,
...
const dedupeDishesByName = (dishes) => {
    const seen = new Set();
...
const hydrateDishTemplates = async ({ templates, context, placeId, restaurantImageUrl, restaurantName, gallery, placeLogger, }) => {
    ...
    const hydrated = new Array(templates.length);
    const pending = [];
    ...
    const processTemplate = async (template, index) => {
        ...
        const canUsePhoto = Boolean(templatePhotoReference) &&
        ...
        if (canUsePhoto) {
            ...
            try {
                ...
                if (!photoData) {
                    ...
                    if (context.visionClient?.classifyDishPhoto) {
                        try {
                            ...
                            if (classification) {
                                ...
                                const passesThreshold = classification.is_dish && (classification.confidence ?? 1) >= minDishConfidence;
                                if (!passesThreshold) {
                                    ...
                                    await upsertRawPhotoRecord(context.supabase, placeLogger, {
                                        placeId,
                                        photoReference: templatePhotoReference,
                                        width: photoData.width,
                                        height: photoData.height,
                                        hash: photoHash,
                                        classification,
                    ...
                    const uploadedUrl = await uploadDishPhoto(context.supabase, photoData, placeId, templatePhotoReference, context.bucket, placeLogger);
                    if (uploadedUrl) {
                        ...
                        gallery.push({
                            ...
                            type: 'dish',
                            label: template.name,
                            source_type: template.source_type ?? 'photo',
                        });
                    }
                    else {
    ...
    const enqueue = async (template, index) => {
...
export const generateRegionKey = ({ latitude, longitude, radius, keyword, }) => {
...
export const upsertRegion = async (supabase, region, logger) => {
...
const enqueueReviewTasks = async (supabase, logger, subjectType, records, runId, priority) => {
...
const persistDishVersions = async (supabase, logger, dishes) => {
    ...
    const currentVersionByDish = new Map();
...
export const persistRestaurantsAndDishes = async (context, restaurantRecords, dishTemplates) => {
    ...
    if (!context.dryRun && restaurants && restaurants.length > 0) {
        await enqueueReviewTasks(context.supabase, scopedLogger, 'restaurant', restaurants, context.runId, context.reviewPriority);
    }
    const restaurantByPlaceId = new Map((restaurants || []).map((restaurant) => [restaurant.place_id, restaurant]));
    const dishesPayload = dishTemplates
        ...
        const completeness = computeDishCompleteness({
            ...
            price: dish.price,
            optionSets: dish.option_sets ?? null,
        });
        return {
            ...
            photo_tags: (dish.photo_insight?.tags && dish.photo_insight.tags.length > 0 ? dish.photo_insight.tags : null) ??
                (dish.photo_classification?.tags && dish.photo_classification.tags.length > 0
                    ? dish.photo_classification.tags
                    : null),
            option_sets: dish.option_sets ?? null,
    ...
    if (!context.dryRun && dishes.length > 0) {
        ...
        await enqueueReviewTasks(context.supabase, scopedLogger, 'dish', dishes, context.runId, context.reviewPriority);
    }
    return {
...
const loadRestaurantsForRegion = async (context, regionRecord) => {
    ...
    const restaurantRecords = new Map();
    const dishTemplates = [];
    let processed = 0;
    const detailsMap = await context.googleClient.getPlaceDetailsBatch(searchResults.map((place) => place.place_id), {
        ...
        try {
            ...
            if (details.photos && details.photos.length > 0) {
                try {
                    ...
                    imageUrl = await uploadRestaurantPhoto(context.supabase, photoData, details.place_id, context.bucket, placeLogger);
                    if (imageUrl) {
                        gallery.push({
                            url: imageUrl,
                            type: 'restaurant',
            ...
            const { matchedReviews, remainingPhotos } = matchReviewDishesToPhotos(dishGroups.reviewDishes, [...photoDrivenDishes, ...externalPhotoDishes]);
            ...
            placeLogger.warn('Skipping place due to error', { error: error.message });
        }
    }
    ...
    }, restaurantRecords, dishTemplates);
};
export const loadRestaurantsFromGooglePlaces = async (supabase, options) => {
    const apiKey = options.apiKey || globalThis?.process?.env?.GOOGLE_MAPS_API_KEY;
    if (!apiKey) {
...
export const createRegionIfMissing = async (supabase, region, logger = defaultLogger) => {
