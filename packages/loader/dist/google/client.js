...
import { GooglePlacesError, buildGoogleUrl, delay, executeWithBackoff, fetchGoogleJson, formatLocation, } from './common.js';
...
export class GooglePlacesClient {
    ...
    async nearbySearch(options) {
        ...
        do {
            ...
            const body = await fetchGoogleJson('nearbysearch', params, this.apiKey);
    ...
    async getPlaceDetails(placeId, options = {}) {
        ...
        const body = await fetchGoogleJson('details', params, this.apiKey);
    ...
    async getPlaceDetailsBatch(placeIds, options = {}) {
        ...
        const entries = await Promise.allSettled(placeIds.map((placeId) => limiter(() => this.getPlaceDetails(placeId, options).then((details) => [placeId, details]))));
        const map = new Map();
    ...
    async fetchPhoto(photoReference, options = {}) {
        if (!photoReference) {
            return null;
        }
        return executeWithBackoff(async () => {
            const url = buildGoogleUrl('photo', {
            ...
            if (response.status === 429 || response.status >= 500) {
                throw new GooglePlacesError(`Google Places photo request failed with status ${response.status}.`, true);
            }
            if (!response.ok) {
                throw new GooglePlacesError(`Google Places photo request failed with status ${response.status}.`, false);
            }
