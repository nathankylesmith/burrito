...
const longestSubstringRatio = (a, b) => {
    if (!a || !b)
        return 0;
...
const computeNameScore = (target, candidate) => {
...
const mergeReviewAndPhotoDish = (reviewDish, photoDish) => {
...
const candidateNames = (dish) => {
    const names = new Set();
    if (dish.name)
        names.add(dish.name);
    if (dish.photo_insight?.dish_guess)
        names.add(dish.photo_insight.dish_guess);
...
const computeDishMatchScore = (reviewDish, photoDish) => {
...
export const matchReviewDishesToPhotos = (reviewDishes, photoDishes) => {
