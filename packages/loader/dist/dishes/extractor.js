...
const createReviewDishes = (details, options, cuisines, maxCount) => {
    ...
    for (const review of reviews) {
        ...
        for (const name of names) {
            ...
            const capturedAt = typeof review?.time === 'number'
...
const createMenuDishes = (details, cuisines, priceLevel, maxCount) => {
    const menus = [];
    const cuisineList = cuisines.length > 0 ? cuisines : deriveCuisines(details);
    cuisineList.slice(0, maxCount).forEach((cuisine, index) => {
        menus.push({
            placeId: details.place_id,
...
export const createFallbackDishes = (details, cuisines, priceLevel, fallbackImageUrl, photoReference) => {
    const fallbackCuisine = cuisines.length > 0 ? cuisines : [details?.name || 'House'];
    return fallbackCuisine.slice(0, 2).map((cuisine, index) => ({
        placeId: details.place_id,
...
export const createReviewDishTemplates = (details, options, cuisines, priceLevel) => createReviewDishes(details, {
    fallbackImageUrl: options.fallbackImageUrl,
    fallbackPhotoReference: options.fallbackPhotoReference,
    priceLevel,
}, cuisines, options.maxReviewDishes ?? MAX_REVIEW_DISHES);
export const createMenuDishTemplates = (details, cuisines, priceLevel) => createMenuDishes(details, cuisines, priceLevel, MAX_MENU_DISHES);
export const createDishTemplates = (details, options) => {
    if (!details?.place_id) {
        return {
            reviewDishes: [],
            menuDishes: [],
    ...
    const fallbackDishes = reviewDishes.length === 0 && menuDishes.length === 0
