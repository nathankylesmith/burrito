...
describe('formatLocation', () => {
    ...
    it('throws for invalid data', () => {
        expect(() => formatLocation({ lat: 1 })).toThrow();
    });
});
describe('uploadPhotoToStorage', () => {
    it('returns null when no photo data provided', async () => {
        ...
        const url = await uploadPhotoToStorage(supabase, null, 'bucket', 'path.jpg', logger);
        expect(url).toBeNull();
    ...
    it('uploads and returns public url', async () => {
        const supabase = createSupabaseStub();
        const logger = createLoggerStub();
        const url = await uploadPhotoToStorage(supabase, {
            buffer: new Uint8Array([1, 2, 3]),
            contentType: 'image/jpeg',
        }, 'dish-images', 'restaurants/abc.jpg', logger);
        expect(url).toBe('https://example.com/photo.jpg');
        expect(supabase.storage.from).toHaveBeenCalledWith('dish-images');
        expect(supabase.upload).toHaveBeenCalledWith('restaurants/abc.jpg', expect.any(Uint8Array), expect.objectContaining({ contentType: 'image/jpeg', upsert: true }));
    ...
    it('returns null when upload fails', async () => {
        ...
        }, 'dish-images', 'restaurants/fail.png', logger);
        expect(url).toBeNull();
