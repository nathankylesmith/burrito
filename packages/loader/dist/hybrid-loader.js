...
const convertWebpToJpeg = async (input) => {
...
export class HybridLoader {
    constructor(options) {
        ...
        if (options.visionModel) {
            const visionProvider = (options.visionProvider ?? 'local').toLowerCase();
        ...
        if (options.llmModel) {
            const provider = (options.llmProvider ?? 'local').toLowerCase();
    ...
    async getBasicRestaurantList(logger) {
        ...
        if (this.googleClient) {
            try {
                ...
                const placeIds = searchResults.map((place) => place.place_id).filter(Boolean);
                let detailsMap = null;
    ...
    async processRestaurant(restaurant, logger) {
        ...
        if (menuData && this.menuAgent) {
            try {
                ...
                const menuResult = await this.menuAgent.ingestMenu({ name: normalizedRestaurant.name }, [normalizedRestaurant.website || '']);
                if (menuResult.accepted.length > 0) {
                    dishes = this.convertMenuItemsToDishTemplates(menuResult.accepted[0].text || '', normalizedRestaurant, placeLogger);
                }
                else if (menuText) {
        ...
        if (websiteImages.length > 0 && this.visionClient) {
            try {
                ...
                const visionDishes = await this.processImagesWithLocalVision(websiteImages, normalizedRestaurant, placeLogger);
        ...
        if (missingImages.length > 0 &&
            ...
            try {
                ...
                await this.fillMissingImagesWithGooglePlaces(missingImages, normalizedRestaurant, placeLogger);
            }
            catch (error) {
        ...
        const restaurantRecord = this.buildRestaurantRecord(normalizedRestaurant.place_id, normalizedRestaurant, dishes, stats);
    ...
    async scrapeRestaurantWebsite(restaurant, logger) {
    ...
    extractImagesFromHtml(html, baseUrl) {
        ...
        let match;
        const normalizedBase = baseUrl && typeof baseUrl === 'string' && /^https?:\/\//i.test(baseUrl) ? baseUrl : undefined;
    ...
    deduplicateDishes(dishes, logger) {
        const seen = new Map();
        const duplicates = [];
        for (const dish of dishes) {
            ...
            if (seen.has(key)) {
                ...
                const shouldReplace = ((dish.confidence || 0) > (existing.confidence || 0) ||
    ...
    convertMenuItemsToDishTemplates(menuText, restaurant, logger, isFallback = false) {
    ...
    async processImagesWithLocalVision(imageUrls, restaurant, logger) {
        const dishes = [];
        ...
        for (const imageUrl of imageUrls.slice(0, 5)) {
            try {
                ...
                if (classification?.is_dish && (classification.confidence ?? 0) > (this.options.minDishPhotoConfidence || 0.35)) {
                    ...
                    const uploadedUrl = await uploadDishPhoto(this.options.supabase, photo, restaurant.place_id || 'unknown', createHash('sha1').update(imageUrl).digest('hex'), 'dish-images', logger);
                    if (uploadedUrl) {
    ...
    findDishesWithoutImages(dishes) {
        return dishes.filter(dish => !dish.image_url);
    }
    async fillMissingImagesWithGooglePlaces(dishes, restaurant, logger) {
        if (!restaurant.place_id || !this.googleClient)
            return;
        try {
                    ...
                    try {
                        ...
                        if (this.visionClient) {
                            ...
                            if (classification.is_dish && classification.confidence > 0.5) {
                                ...
                                if (dishWithoutImage) {
                                    const uploadedUrl = await uploadDishPhoto(this.options.supabase, photoData, restaurant.place_id || 'unknown', photoRef || 'unknown', 'dish-images', logger);
                                    if (uploadedUrl) {
    ...
    async persistProcessedRestaurants(processedResults, logger) {
    ...
    buildRestaurantRecord(placeId, restaurant, dishes, stats) {
    ...
    buildStats(dishes, imagesProcessed) {
...
export async function loadRestaurantsHybrid(options) {
