...
export const deriveCuisines = (details: any): string[] => {
  ...
  return (details?.types || [])
    .filter((type: string) => !nonCuisineTypes.has(type))
    .map(titleCase)
    .slice(0, 3);
};

export const mapPriceLevelToRange = (priceLevel: number | null | undefined): string | null => {
...
export const mapPriceLevelToDishPrice = (
  priceLevel: number | null | undefined,
  variantIndex: number = 0
): number => {
  const base =
    typeof priceLevel === 'number' && priceLevel >= 0 && priceLevel < DISH_BASE_PRICES.length
