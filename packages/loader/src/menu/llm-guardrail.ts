...
export class LlmGuardrailModel implements MenuGuardrailModel {
  private readonly llm: LlmClient;

  private readonly options: LlmGuardrailOptions;
  ...
  private buildPrompt(context: RestaurantContext, page: ScrapedMenuPage) {
    const lines = [
  ...
  async evaluatePage(
    ...
    const action: MenuGuardrailAction =
      parsed.action === 'accept' || parsed.action === 'redirect' ? parsed.action : 'reject';
    ...
    return {
      ...
      normalizedMenuUrl:
        typeof parsed.normalized_menu_url === 'string' ? parsed.normalized_menu_url.trim() || null : null,
