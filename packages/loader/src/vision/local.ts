...
class LocalVisionClient implements VisionClient {
  private readonly endpoint: string;

  private readonly model: string;

  private readonly options: LocalVisionOptions;

  constructor(options: LocalVisionOptions) {
    if (!options.model) {
  ...
  private async generateResponse(prompt: string, photoBase64: string, attemptMeta: Record<string, unknown>) {
  ...
  async classifyDishPhoto(params: ClassifyDishPhotoParams) {
  ...
  async describeDishPhoto(params: DescribeDishPhotoParams): Promise<DishPhotoInsight | null> {
    ...
    const insight: DishPhotoInsight = {
      model: this.model,
      ...
      price_tier: (pickString('price_tier', 'priceTier', 'price_level', 'priceLevel') as DishPhotoInsight['price_tier']),
...
export const createLocalVisionClient = (options: LocalVisionOptions): VisionClient =>
  new LocalVisionClient(options);
